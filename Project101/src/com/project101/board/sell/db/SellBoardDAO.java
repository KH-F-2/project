...
public class SellBoardDAO {
	...
	public int boardInsert(SellBoardBean boardBean) {
		try {
			...
			String sql = "insert into SELL_BOARD " + "(SB_NO, SB_WRITER, SB_PDATE, SB_TITLE, "
					+ "SB_CONTENT, SB_PRICE, SB_DATE, SB_READCOUNT) " + "values(?, ?, ?, ?, ?, ?, sysdate, ?)";
			
			pstmt = conn.prepareStatement(sql);
			
			pstmt.setInt(1, boardBean.getSB_NO());
			pstmt.setString(2, boardBean.getSB_WRITER());
			pstmt.setDate(3, boardBean.getSB_PDATE());
	...
	public SellBoardBean getDetail(int num) {
		...
		try {
			...
			if (rset.next()) {
				boardBean.setSB_NO(rset.getInt("SB_NO"));
				boardBean.setSB_WRITER(rset.getString("SB_WRITER"));
				boardBean.setSB_PDATE(rset.getDate("SB_PDATE"));
				boardBean.setSB_MDATE(rset.getDate("SB_MDATE"));
				boardBean.setSB_TITLE(rset.getString("SB_TITLE"));
				boardBean.setSB_CONTENT(rset.getString("SB_CONTENT"));
				boardBean.setSB_PRICE(rset.getInt("SB_PRICE"));
				boardBean.setSB_DATE(rset.getDate("SB_DATE"));
				boardBean.setSB_READCOUNT(rset.getInt("SB_READCOUNT"));
				return boardBean;
			}
	...
	public int boardModify(SellBoardBean sellboard) {
		try {
			...
			String sql = "update SELL_BOARD " + "set SB_TITLE=?, SB_PRICE=?, SB_CONTENT=?, SB_PDATE=?, SB_MDATE=sysdate "
					+ "where SB_NO=?";
