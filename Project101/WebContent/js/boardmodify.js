$(document).ready(function(){
	...
	$('#modify_submit').submit(function () {
		board_type = $('input[type="radio"]:checked').val();
		if(board_type == 1){
			$('#modify_submit').attr('action', "./pbmodifyAction.pb?PB_NO="+$('#PB_NO').val());
		} else {
			$('#modify_submit').attr('action', "./sbwriteaction.sb");
		}
...
function initMap() {
	seoulCityhall = {
		lat : parseFloat($('#markerLat').val()),
		lng : parseFloat($('#markerLng').val())
	};
	...
	autocomplete.addListener('place_changed', function() {
		...
		if (place.geometry) {
			...
			mentById('autocomplete').placeholder = 'Enter a city';
		}
	});
